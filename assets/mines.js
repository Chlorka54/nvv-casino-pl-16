...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-ec21ae7a";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-ec21ae7a">'+'<span class="tile-face-ec21ae7a tile-front-ec21ae7a"></span>'+'<span class="tile-face-ec21ae7a tile-back-ec21ae7a"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ec21ae7a")){return;}
var back=tile.querySelector(".tile-back-ec21ae7a");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-ec21ae7a");if(isMine){tile.classList.add("boom-ec21ae7a");}
if(dim){tile.classList.add("dimmed-ec21ae7a");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-ec21ae7a")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-ec21ae7a");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-ec21ae7a")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-ec21ae7a");void panelEl.offsetWidth;panelEl.classList.add("jolt-ec21ae7a");}
endRound();}
